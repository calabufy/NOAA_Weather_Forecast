...
from app.db.historical import HistoricalModelDay
from scripts.backfill_historical_metrics import (
    ACTUAL_SOURCE,
    FORECAST_SOURCE,
    build_metric_rows,
    parse_iem_rows,
...
def test_historical_tables_do_not_touch_live_tables():
    ...
    daily = [
        HistoricalModelDay(
            ...
            forecast_tmax_f=74.0,
            actual_tmax_f=73.0,
            forecast_source=FORECAST_SOURCE,
            actual_source=ACTUAL_SOURCE,
        )
    ]
    metric_rows = build_metric_rows(daily, date(2025, 7, 16), date(2025, 7, 16))

    assert sqlite_repo.upsert_historical_days(conn, daily) == 1
    assert sqlite_repo.upsert_historical_metrics(conn, metric_rows) == 1
    assert sqlite_repo.historical_error_series(
